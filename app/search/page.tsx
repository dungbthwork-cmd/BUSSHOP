...
import { MapPin, Clock3, BadgePercent } from "lucide-react"
...
export default function SearchPage() {
  ...
  const filtered = (data ?? [])
    .map((r) => ({
      ...
      promo: Math.random() > 0.5 ? "GIẢM 10% hôm nay" : "",
    }))
    .filter((r) => {
      ...
      const hourOk = departureHours.length
        ? departureHours.includes(hour)
        : true
  ...
  const sorted = useMemo(() => {
    ...
    if (sort === "price") arr.sort((a, b) => a.price - b.price)
      ...
      arr.sort(
        (a, b) =>
          Number(a.departAt.split(":")[0]) -
          Number(b.departAt.split(":")[0]),
      )
  ...
  useEffect(() => {
    setPage(1)
  }, [from, to, date, brands, departureHours, priceRange])

  /** QUAN TRỌNG: push đầy đủ params sang /checkout */
  const handleSeatContinue = (selectedSeats: number[]) => {
    if (!selecting || !selectedSeats.length) return

    const params = new URLSearchParams({
      routeId: selecting.id,
      from: selecting.from,
      to: selecting.to,
      date: selecting.date,
      departAt: selecting.departAt,
      price: String(selecting.price),
      brand: selecting.brand,
      seats: selectedSeats.join(","), // "1,2" hoặc "B12,B13"
    })
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        {item.departAt} • {Math.round(item.durationMin / 60)}h
                      </span>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                    <div className="flex items-center gap-2 text-xs">
                      <Badge variant="outline">
                        Còn khoảng {item.seats - item.taken.length} ghế
                      </Badge>
                    </div>
        ...
        route={
          selecting
            ? {
                ...
                from: selecting.from,
                to: selecting.to,
                // KHÔNG truyền date ở đây để tránh lỗi type, chỉ dùng trong params
                departAt: selecting.departAt,
      ...
      <Dialog open={liveOpen} onOpenChange={setLiveOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 text-sm">
            <p>
              Xe {liveRoute?.brand} tuyến {liveRoute?.from} →{" "}
              {liveRoute?.to}
            </p>
            <div className="flex h-40 items-center justify-center rounded-xl bg-gradient-to-br from-sky-50 to-emerald-50 text-xs text-muted-foreground">
              Bản đồ minh họa (placeholder)
            </div>
            <p className="text-xs text-muted-foreground">
              Mô phỏng UI, không phải dữ liệu thật.
            </p>
          </div>
        </DialogContent>
      </Dialog>
